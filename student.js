...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function submitQuizAnswers() {
    ...
    } catch(e) { alert("Network error submitting quiz."); }
  ...
  window._takeQuiz = (id) => {
    ...
    if (quiz) openTakeQuizModal(quiz);
  };

  async function loadStudentQuizzes() {
    if (!sQuizzesList) return;
    try {
      const res  = await fetch("fetch_all_quizzes_student.php");
      const data = await res.json();
        ...
        <div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:10px;padding:10px;margin-bottom:6px;">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;">
            <div style="flex:1;">
              ...
              <span style="font-size:10px;color:#94A3B8;display:block;">By ${quiz.teacher_name||'Teacher'} • ${quiz.duration_mins} min • ${quiz.questions.length} Q</span>
            </div>
            <button onclick="window._takeQuiz(${quiz.id})"
              style="background:#DCFCE7;color:#059669;border:none;border-radius:6px;padding:4px 10px;font-size:11px;font-weight:600;cursor:pointer;flex-shrink:0;margin-left:6px;">▶ Take Quiz</button>
          </div>
        </div>`).join('');
